...
#if DEBUG_ABARRAY_HANDLES
static HashPtr *S_ADECL(ABARRAY_tracker); // every array in existence 
...
static ArrayHdr* S_ADECL(re_capacity)(ArrayHdr *a, int cap DBG_PARMS) 
{
    ...
    if(!a)
        return S_ADECL(fill)(ABMALLOC_DBGCALL(ABARRAY_MEMSIZE(cap)), ABARRAY_MEMSIZE(cap), 0 DBG_PARMS_CALL);
    ...
    if(a->cap < cap)
    {
		...
		{
			res = (ArrayHdr*)ABMALLOC_DBGCALL(ABARRAY_MEMSIZE(cap));
			memcpy(res, a, ABARRAY_MEMSIZE(a->cap));
		}
		else
		{
			res = (ArrayHdr*)ABREALLOC_DBGCALL(a,ABARRAY_MEMSIZE(cap));
		}
		if(a != res)
		{
...
{
    return T_FROM_ABARRAY(S_ADECL(re_capacity)(NULL,capacity DBG_PARMS_CALL));
}
...
{
    ArrayHdr *d = ABARRAY_FROM_T(*hdest);
    TYPE_T *offset;
    int new_size;
    int remain;
    ABARRAY_CHECK_HANDLE(d);
