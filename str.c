...
char* Str_temp_dbg(void *mem, size_t mem_size DBG_PARMS)
{
	char *str;
    if(mem)
        str = achr_init_dbg(mem, mem_size, ARRAYFLAG_ALLOCA DBG_PARMS_CALL);
    else
        str = achr_create(mem_size);
...
int Str_len_dbg(Str *hstr DBG_PARMS)
{
    ...
    if(hstr && *hstr)
    {
        ...
        assertm(n == sl,"string length %i doesn't match Str len %i", sl, n);
    }
#endif
...
char *Str_ncpy_dbg(Str *hres,Str *src, int n DBG_PARMS)
{
    n = MIN(n,Str_len(src));
...
char* Str_dup_dbg(const char *src DBG_PARMS)
{
	char *ret = NULL;
